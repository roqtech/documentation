...
that  is executed as a cron job. For these scenarios, ROQ provides a special user called "service account". This
user  has super-admin permissions, so we recommend using it carefully.

The service account user is always available on ROQ Platform side and isn't shown in the console. For security
reasons it can only be used from the server side of your application.
...
<Tabs items={['Node.js SDK']}><Tab>
```typescript
await client.asUser(userId).notifications.sendMail(...)
```
</Tab></Tabs>

To interact with ROQ Platform using the service account, you can use the `asServiceAccount` function:

<Tabs items={['Node.js SDK']}><Tab>
```typescript
await client.asServiceAccount().notifications.sendMail(...)
```
</Tab></Tabs>
...
<Tabs items={['Node.js SDK']}><Tab>
```typescript
const response = await client.asUser(userId).user.users({ 
    limit: 10, 
    offset: 0, 
