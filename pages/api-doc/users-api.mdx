...
the [users feature guide](/feature-guides/users) for more information and instructions.
...
You can find the full API doc of this mutation [here](https://snapshotp.roq-platform.com/docs/#mutation-createUser).
...
| `active`    | boolean | Set to true if the user is activate and able to login to your application.                                                                                                                         |
| `tenantId`  | UUID    | ID of the tenant which you created using the [`createTenant()`](/api-doc/tenants-api#createtenant) API                                                                                             |
...
You can fetch all users using the `users()` query. All [query parameters](/api-doc/graphql-basics-api) can be applied
