...
Any web application needs user authentication. ROQ BaaS offers easy authentication through its SDK. For Next.js projects, you can utilize the `signIn`, `signOut`, and `signUp` methods from the `defaultBrowserClient` factory.
...
To add a login button to a Next.js page, we can use the `signIn` API from the generated SDK. The `defaultBrowserClient` factory live in the `src/lib/roq/roq-client.ts"`. To use it you must import it first:
...
export default function Home() {
  return (
    <main className={`text-center flex flex-col items-center justify-between p-24`}>
      ...
      <button 
          onClick={() => defaultBrowserClient.signIn()}
...
```
If you want to redirect the page after the login process, you should use `postLoginRedirect` option. 
...
export default function Home() {
  ...
  const handleSignIn = () => {
      defaultBrowserClient.signIn('owner', { postLoginRedirect: '/user' })
  }
...
export default function Home() {
  return (
    <main className={`text-center flex flex-col items-center justify-between p-24`}>
      <h1 className='m-10'>Welcome</h1>
      <button 
        onClick={() => defaultBrowserClient.signUp()}
        className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
      >
        Signup
      </button>
...
If we want to redirect the page after signing up, you need to configue the `postLoginRedirect` option.
...
export default function Home() {
  ...
  const handleSignUp = () => {
      defaultBrowserClient.signUp('owner', { postLoginRedirect: '/user' })
  }
...
const AuthenticatedSection = () => {
  ...
  return (
    <div className='m-5'>
        ...
        <br />
        <button
          onClick={() => defaultBrowserClient.signOut()}
          className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
        >
          Logout
        </button>
...
The `useRoqPlatformUserProfiles()` is a hook to get the user's profile data. It support filtering, sorting and searching for a specific user profile data.
