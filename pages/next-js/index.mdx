...
export default function Home() {
  ...
  const handleSignUp = async () => {
    await roqBrowserClient.signUp('REPLACE', { postLoginRedirect: 'http://localhost:3000/user' });
  };

  const handleSignIn = async () => {
    await roqBrowserClient.signIn('REPLACE', { postLoginRedirect: 'http://localhost:3000/user' });
  };
