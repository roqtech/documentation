...
export default function Home() {
  ...
  const handleSignUp = async () => {
    await defaultBrowserClient.signUp('REPLACE', { postLoginRedirect: '/user' });
  };

  const handleSignIn = async () => {
    await defaultBrowserClient.signIn('REPLACE', { postLoginRedirect: '/user' });
  };
...
In a parallel manner, the `signIn()` method will redirect the user to the login form for the sign-in process. To specify the login form variant, substitute the `REPLACE` placeholder with the corresponding key obtained from the project console.
...
```tsx filename="src/pages/user/page.tsx"
'use client'

import { defaultBrowserClient } from '@/lib/roq/roq-client';
import { useRoqPlatformUserProfiles, useUserFindMany, useRoqPlatformRoles } from "@/lib/roq";
...
export default function Home() {
  ...
  const handleSignUp = async () => {
    await defaultBrowserClient.signUp('owner', { postLoginRedirect: '/user' });
  };

  const handleSignIn = async () => {
    await defaultBrowserClient.signIn('owner', { postLoginRedirect: '/user' });
  };
