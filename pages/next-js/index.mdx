...
export default function Home() {
  ...
  const handleSignUp = async () => {
    await defaultBrowserClient.signUp('REPLACE', { postLoginRedirect: 'http://localhost:3000/user' });
  };

  const handleSignIn = async () => {
    await defaultBrowserClient.signIn('REPLACE', { postLoginRedirect: 'http://localhost:3000/user' });
  };
