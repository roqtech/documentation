...
ROQ SDK for Next.js also provides a `FileUpload` component to make the file upload easy to code. 
Let's take an example of how to upload an image file using ROQ BaaS.

## Using the `FileUpload` component

First, we need to instantiate the file upload component from the `@roq/nextjs` package:
...
The `useRoqFileUploader()` is a hook used in Next.js to upload files. We can use this method to add handlers for file upload events.
...
import { requireNextAuth, useSession, FileUpload, useRoqFileUploader } from '@roq/nextjs'
