...
The process starts with a user's decision to send an invitation. To send any invitation later, we should know about which user sending the invitation and the tenant. 

In Next.js, we can get the user data from session:
...
Another way, we can query these data using the `findMany()` on `user` entity. For example, we can find a user based on the email:
...
const findUserInvitebyEmail = async() => {
    const inviteeData = await roqClient.roqPlatform.userInvites({
		    filter: {
				    email: {
						    equalTo: "sinau.online.terus@gmail.com"
				    }
				}
		})
    return inviteeData
...
If the invitee accept the invitation, the response `status` will have `accepted` value:
...
<Callout type="info">
Please note that the `userInvites()` method is part of the ROQ platform API and cannot be generated. You can read the documentation of the `userInvites()` API [here](https://jupiter-pp.roq-platform.com/docs/#query-userInvites).
</Callout>
