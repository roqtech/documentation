...
In the application development, sometime you need automation for translations such as bulk upload. You can do bulk upload translations in ROQ by using  the Translations API and you can use the function [`upsertTranslationKeys()`](https://jupiter-pp.roq-platform.com/docs/#mutation-upsertTranslationKeys) for this purpose. 
...
You can use `upsertTranslationKeys()` method and Node.js-based parser for bulk upload translations. For instance, you can write the translations in CSV or JSON format then parse it before uploading to the ROQ console.
...
If you prefer JSON file for the translation, you customize the code above. For example, the following is JSON translation file:
