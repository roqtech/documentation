...
With this method you can get the translation key based on it's id. For example to find the key for `translationKey` id `052ee336-af23-487f-bce5-3f8c1fa5a972`
...
The `translationKeys()` can query all the translations include with translation keys. Read the documentation of this API [here](https://jupiter-pp.roq-platform.com/docs/#query-translationKeys).
...
With this method you can find a key based on the keyword, for instance if you want to search a key that contain the word "email", you can find the key with this filter.
...
The created key can be checked in the ROQ Console or by using `translationKey()` API.
