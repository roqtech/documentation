...
There are a few changes to the login page's `index.tsx`. We need to replace the role for sign-in and sign-up process with the role we created in the project setup, which is `business-owner`, and change the redirect page `postLoginRedirect` to `/projects`:
...
export default function Home() {
  ...
  const handleSignUp = async () => {
    await roqBrowserClient.signUp('business-owner', { postLoginRedirect: '/projects' });
  };
 
  const handleSignIn = async () => {
    await roqBrowserClient.signIn('business-owner', { postLoginRedirect: '/projects' });
  };
