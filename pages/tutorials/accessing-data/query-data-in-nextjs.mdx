...
The code above will retrieve all projects data from BaaS and sort the data by the `created_at` field in descending order. The data will be rendered in the `<ProjectList>` component.

### Create a project list component

Create a new component called `project-list.tsx` in the `pages` directory. This component is `<ProjectList>` component that we use in the previous code. The component will render the list of projects that have been created by the **Business Owner**.
...
This page use an interface to define the data type for the project list. Open the file called `project-data-types.ts` in the `src/interface` directory. Add this project list interface in the code:
...
We have built a simple project list page for the **Freelancer** role. The page will display all projects that have been created by the **Business Owner**.
