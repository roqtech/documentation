...
To running a local development, you need to copy environment settings. In the ROQ Console go to **Application Details** → **Environments** menu and then **Copy Env File** in the **Local Development** section. Paste the environment settings into the file `.env` in the root of the generated application project root, you need to create this file it doesn't exist.

For instance, `bistro-94be` has these environment settings:
...
The generated application using [Prisma ORM](https://www.prisma.io/) so you can use any database that is supported by Prisma. For this tutorial we will use [PostgreSQL](https://www.postgresql.org/) and then we will add `DATABASE_URL` key value in the `env` file.
...
To create a database for the generated application, we will use **pgAdmin**, a bundled GUI application for administering PostgreSQL. Lets called this database a **bistro**.

![pgAdmin](/pgadmin.png)

With the default PostgreSQL installation and with the database name is **bistro**, the database URL for the generated application is:
