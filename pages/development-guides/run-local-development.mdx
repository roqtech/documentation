...
There are a few steps to run a local development for the generated application.

<Steps>
### Setup Environment Variables

To run a local development, you need to copy the environment settings. In the ROQ Console go to the **Application Details** → **Environments** menu and then **Copy Env File** in the **Local Development** section. Paste the environment settings into the file `.env` in the root of the generated application project root, you need to create this file if it doesn't exist.

For instance, `bistro-94be` has these settings:
...
To create a database for the generated application, we will use **pgAdmin**, a bundled GUI application for administering PostgreSQL. Let's call this database a **bistro**.
...
By default, the local application will be running on `http://localhost:3000`

![Bistro app](/bistro.png)

and for first-time use, you need to create an account to be able to use the generated application.
