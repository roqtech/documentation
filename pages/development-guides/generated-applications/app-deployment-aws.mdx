...
We can use the `psql` CLI tool to test the database connection:
...
Store the certificate bundle `us-east-1-bundle.pem` in an accessible directory. This is only required if connecting to the database outside the AWS network.
