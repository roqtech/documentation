...
First time build will be failed because we haven't setup the environment variables.
</Callout>
...
In AWS Amplify, navigate to **App settings**, select **Environment variables**, and manually enter variables from ROQ Console or `.env` file.
...
AWS Amplify have an application build specification that can be edit trough **App settings** or overridden by `amplify.yaml` file. The build specification YML file controls how an app build will run and is stored on the servers.

To edit buld specification in the AWS Amplify, go to **App settings**, select **Build settings** and edit **App build specification**. 

![build spec](/aws-amplify-build-spec.png)

We can overidde the AWS Amplify setting above with create a `amplify.yaml` file on the application project root directory:
...
Please note that environment variable that previously entered manually are available only in build time. To make the environment variables also available in runtime, we need to save the environment variables into `.env` file.
...
<Callout type="info">
Amplify Hosting supports adding environment variables to your application's builds by setting them in the project's configuration in the Amplify console. However, a Next.js server component doesn't have access to those environment variables by default. For more information, please read it [here](https://docs.aws.amazon.com/amplify/latest/userguide/ssr-environment-variables.html).
</Callout>

### Re-deploy Application

After the environment variables setup, we can re-deploy the application in Amplify Hosting. Click the application name. For example, `anime-figure-store`:

![aws amplify main branch](/aws-amplify-build-main.png)

Click the branch `main` that will bring us into the branch build details.

![aws amplify main branch deploy](/aws-amplify-build-deploy.png)

To force build and redeploy, click the **Redeploy this version** button.
...
Signup to the deployed application and every time we make a changes to the source code and then push it to the GitHub repository. AWS Amplify Hosting will pick up the changes and build the new version of the application. 

</Steps>

## ROQ Environment

To make ROQ-generated application working on a PaaS, it is necessary to configure specific environment settings for that PaaS. You need to create a new environment on [ROQ Console](https://console.roq.tech/).
...
From our Amplify Hosting build, we already have information for the Base URL. In the ROQ Console, go to the **Application Details** → **Settings** to configure the environment variables.
...
If the deployed application is not running as expected, such as an `Internal Server Error`, please check the compute logs. On AWS Amplify, go to the **App settings**, then select **Monitoring** and finally select **Hosting compute logs**.
