...
ROQ's generated application need PostgreSQL by default and Heroku already provides an add-on. Go to the **Resources** tab in the Heroku Dashboard and find "PostgreSQL" from Heroku.
...
We can set the environment variables manually or create an automated scripts for that. For example, to automate it using Node.js:
...
```
The we can run in the root project folder (where the `.env` reside).
...
The last step is push the generated application code into Heroku repository. This command will automatically build the generated application on Heroku:
...
If everything is set up correctly, we can run the installation test for our application on Heroku.
...
If the generated application database schema not deployed correctly on PostgreSQL, we can re-run `prisma migrate` using Heroku CLI. For example: 
...
For other troubleshooting, please look into our [Troubleshooting](/faq) documentation section.