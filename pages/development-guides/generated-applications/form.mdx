...
1. Add the `description` property to the `BookInterface` interface in the `interfaces/book` module. The updated code would look like this:

```ts filename="/src/interfaces/book/index.ts"
...
```ts filename="/src/validationSchema/books/index.ts"
...
```jsx filename="src/pages/book/create/index.tsx"
...
To do this you need to update the JSX code in the `BookEditPage` component in `pages/books/edit/[id]` directory. **Add** the following code inside the `<form>`:

```jsx filename="src/pages/books/edit/[id]/index.tsx"
<FormControl id="description" mb="4" isInvalid={!!formik.errors?.description}>
    <FormLabel>Description</FormLabel>
    <Input type="text" name="description" value={formik.values?.description} onChange={formik.handleChange} />
    {formik.errors.description && <FormErrorMessage>{formik.errors?.description}</FormErrorMessage>}
...
The book data is displaying in the Book list. To show book `description` on the list, you need to change `BookViewPage` component in `pages/books/view/[id]` directory.

Update JSX in the `BookViewPage`and **add** this code inside the `<Stack>` component:

```jsx filename="src/pages/books/view/[id]/index.tsx"
