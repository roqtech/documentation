...
The generated application uses API endpoints in `pages/api` source directory. These APIs are the bridges between the UI components, ROQ platform and other third party services.

To add API endpoint is easy, you can do so by creating a function inside `pages/api` source directory that has the following format:
...
```js filename="src/pages/hello.tsx"
...
```ts filename="src/components/hello/index.tsx"
...
```ts filename="src/pages/hello.tsx"
import { requireNextAuth } from '@roq/nextjs';
import { useRouter } from 'next/router';
import AppLayout from 'layout/app-layout';
import { Box } from '@chakra-ui/react';
