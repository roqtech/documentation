...
Before uploading the application to Google App Engine, it is important to configure the project. Create `app.yaml` file in the generated application root folder and fill with these contents:
...
```
This file specifies that the project uses runtime Node.js version 18 and the standard environment for Google App Engine. It also specifies the entry point or script to run after uploading the files.

### ROQ Environment

ROQ's generated application need environment variables from the ROQ Console. We can create a new environment for the Google App Engine.

<ROQEnv/>

![roq console for google app engine](/google-app-engine-roq-env.png)

We can also copy the environment variables from other environment. For example to copy environment variables from **Preview** environment. Go to **Environment Sync** menu and then click the **Copy Configuration** button.
...
Google App Engine have this following format URL for the default service:
...
The base URL need to be setup in the ROQ Console. Go to the **Application Details** → **Settings**.
...
Create environment `.env` file if don't exist in the generated application root folder then copy all the environment variables from the ROQ Console and add the `DATABASE_URL` from our previous PostgreSQL database setup:
...
Before uploading the code to the Google App Engine, first we must **build** the project locally:

```shell
npm run build
```

It is necessary to run the Prisma migrate on the PostgreSQL database for the generated application to function properly. While other platforms, such as Vercel and Heroku, perform this step automatically along with the build process, there is no such automation available on Google App Engine.

To use the PostgreSQL database we set up earlier, we need to manually run the migration:
...
We can check the Google App Engine logs to confirm successful upload:
