...
The deployment will be error because we haven't set the environment settings yet. This is the fastest way to get the project domain name from Vercel that we will need later for the project environment settings. 
</Callout>
...
We will use PostgreSQL database. This database is supported by Vercel and it's quite easy to setup.

![Create vercel database](/vercel-create-database.png)

To create **Postgres** database click the **Create** button then follow along, give the database name, any name, for example **bristodb** then click **Create**.

![Create postgres database](/vercel-create-database-postgres.png)

Vercel will populate PostgreSQL credentials and some integrations. What we should pay atention to is the Prisma integration.
