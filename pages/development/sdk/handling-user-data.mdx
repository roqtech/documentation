...
Creating new users is only possible through the ROQ Platform. When a new user is created on the ROQ Platform, a corresponding user account is also created on the BaaS. You can create a new user by sending an invitation from the Console, user signup or by using the `createUser` API.
...
Update user data can be done on both the ROQ Platform and BaaS. We can update it using the Console or BaaS API.
