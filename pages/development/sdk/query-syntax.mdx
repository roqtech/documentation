...
To retrieve all user profiles, we can use the `userProfiles()` API that specifically query data from the ROQ Platform.
...
To get all files from the users we can use `files()` API. 
...
Suppose we have a user id `8b249b07-bdd9-43f7-a10e-ace10f27ff66` and want to know what the user's roles. We can use the `roles()` API:
