...
Suppose we want to create a chapter for a specific book title and for the return data, we want to also include all the book data. We can use `create()` API to create a new chapter.
...
We can also update the existing data. For example, suppose we want to update a chapter content, we can use `update()` API.
...
The above code response also will include the book data.

### `delete`

The `delete()` API can be used to delete a specific data on any entities. For example, to delete a user review with the id `78e99df7-30d1-4205-b88f-6e804d1ec0fc`:
...
Suppose we want to find a book with the genre contain the "Science" word. We can use the `findMany()` on the book entity to search the book.


```tsx
const findBook = async() =>{
	const compData = await roqClient.book.findMany({
	 where: {
		 genre: {
			 contains: "Science"
		 }
...
const dataBook = await roqClient.book.findManyWithCount({
