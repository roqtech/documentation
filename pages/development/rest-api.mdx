...
Since the cookie has the `HttpOnly` flag enabled, we can only retrieve the user token from the server-side. In the Next.js application, we can use `getServerSideProps()` method.
...
This the full example code on how to get the `roq-sesson-token` and use it to query book count data.
...
The less common way to test the REST API from BaaS is to copy the token directly from the browser cookie and then paste it into the authorization header.
