...
To access the REST API in the ROQ BaaS, you'll need a token and the REST API URL, which is set on the `ROQ_API` environment key in the `.env` file. 

A useful tip for identifying the REST API URL is to check if it matches the generated application URL with the word `baas` added to it.

For example, the generated application URL:

```shell
https://myawesome-roq-app.vercel.app
```

then the REST API URL is:

```shell
https://myawesome-roq-app-baas.vercel.app
```

Let's say you have a book story application hosted at `https://book-story-v1-ca2b.vercel.app` and you want to count the total number of available books from the client side.
