...
The `withAuthorization()` will modify the query for the current user ID and user's permissions. For example, to get the list of applications, the `findManyPaginated()` method will be called with this custom filter WHERE condition:
...
Other than `withAuthorization()`, ROQ also provides another layer of security. The `hasAccess()` The hasAccess method ensures that the user can only proceed to perform these CRUD operations if they are authorized to do so.
...
The `hasAccess()` method also will query and return the data based on the user's ID, roles, and permissions.
