...
The access management in ROQ based on query plans which ROQ computes based on the curent user's permissions and the database tables. The [`buildQueryPlan()`](https://jupiter-pp.roq-platform.com/docs/#mutation-buildQueryPlan) method is used to create query plans and this query plans is created on production in the ROQ Platform.
...
Initially, the generated application will check if the query plans is already cached locally. If not the `initQueryPlanCache()` will get the query plans from the ROQ Platform.

<Callout type="info">
The `initQueryPlanCache()` method is in the `@roq\prismajs` package.
</Callout>

The query plans is cached in the `node_modules\@roq\prismajs\dist` directory, in the `query-plans.json` file.
...
The query plans in the generated application is periodically updated from the ROQ Platform but if you need to force update the local query plan, you can do one of the following ways:

### Using CLI
You can use utility `roq-authorization` that already exist upon dependencies package installation
