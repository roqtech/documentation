...
Ths `hasAccess()` API method is used to check a user permission or access to an entity.
...
The code above will return query plans data. For example:
...
| Parameter      | Type   | Description                                 |
| -------------- | ------ | --------------------------------------------|
| `entity` | string | The entity that user want to access to  |
...
The `hasAccess` method is currently implemented on the client side and at the moment, it is only compatible with Next.js framework.
...
| `operation` | enum | The entity permission (project or platform entity) |
