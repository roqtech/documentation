...
* Set permissions and apply them to users.
...
periodically. This way, your application is independent of the availability of the ROQ Platform. Furthermore, there is no need to send all your data. ROQ only needs to know the schema of your database; the actual queries will be performed on your side.
...
A **role** is represented by its name, such as _owner_, _member_, _manager_, _chef_, _driver_, or _customer_. 

### Permissions

**Permissions** determine the access level to a specific type of object. For instance, the role _manager_ may be granted read access to the sales orders of a restaurant management application.
...
**Scope** determine the range of access within a permission. Scope for selected access levels are defined in a similar manner.
