...
Now, suppose we already setup a Next.js app then you can use the `useSession()` hooks  to get the user id and `userProfile()` to get the user profile information. To display the user profile information, let's create a page in a folder  `src/app/profile`, create the directory if it doesn't exist. Then, add the following code to the `page.tsx` file:

<Callout type="info">
This code using Next.js 13 or later with **App Router** feature. To start Next.js app development with ROQ BaaS, please read this [quickstart](/next-js).
</Callout>
...
export default function Profile() {
    ...
    const userId = session?.roqUserId as string;
    ...
    useEffect(() => {
        ...
        const fetchUserProfile = async () => {
            try {
                const response = await roqBrowserClient.roqPlatform.userProfile({ id: userId });
                setUserProfile(response?.userProfile as UserProfile);
            } catch (error) {
    ...
    return (
        <div>
            ...
            <div>
                ...
                <p>Locale: {userProfile.locale}</p>
                <p>Timezone: {userProfile.timezone}</p>
				<p>ROQ ID: {userProfile.id}</p>
            </div>
        </div>
    );
}
...
{/*little description what the code above doing*/}

The data returned by the `userProfile()` API is a user profile object with the following structure:
