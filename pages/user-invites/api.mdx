...
The `acceptUserInvite` API is the ROQ's validation if the invitee accept the invitation. For the full API documentation, please look into this [link](https://jupiter-pp.roq-platform.com/docs/#mutation-acceptUserInvite).
...
{/* Need to check this codes - Equan P.*/}
...
const updateUserInvite = await roqClient.asSuperAdmin().updateUserInvite({
  ...
  userInvite: {
     acceptedByUserId: '89043940ed-9b1f-4819-9092-abd5facc314b',
     firstName: "Adam",
     lastName: "Houdini",
     locale: "de-DE",
     data: { address: "Somewhere ST." }
  }
})
```
</Tab>
...
mutation updateUserInvite(
  $id: ID! = "80c81294-f338-4819-9b1f-acc314babd5f",
...
You can get all the user invites list using this [`userInvites()`](https://jupiter-pp.roq-platform.com/docs/#query-userInvites) method API. You can limit the results and searching the invitee by email, name, or keywords, etc.
...
Get the data of the invitee. The documentation for `userInvite` API is in the [GraphQL API reference](https://jupiter-pp.roq-platform.com/docs/#query-userInvite).
